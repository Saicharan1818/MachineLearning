#goal is to predict instagram views if 1 hour has 750likes
...
print(df.head())
print("\nBasic statistics:")
print(df.describe())  #to get basic statistics like mean, std, min, max

# Step 4: Visualize the Data
plt.style.use('seaborn-v0_8-whitegrid')   #to look clean and professional


plt.figure(figsize=(10, 6))
plt.scatter(df['first_hour_likes'], df['total_views'], c=df['first_hour_likes'], cmap='viridis', s=100, alpha=0.7, edgecolors='white', linewidth=1.5)
...
plt.savefig('Likes_vs_Views_Scatter.png', dpi=150, bbox_inches='tight')
plt.tight_layout()
plt.show()
...
plt.figure(figsize=(10, 6))
plt.scatter(df['first_hour_likes'], df['total_views'],c="#20c4f1", s=100, alpha=0.7, edgecolors='white', linewidth=1.5, label='Actual Data')

X_line = np.linspace(df['first_hour_likes'].min(), df['first_hour_likes'].max(), 100).reshape(-1, 1)

plt.plot(X_line, model.predict(X_line), color="#54fa12", linewidth=2.5, label='Regression Line')

plt.xlabel('First Hour Likes', fontsize=12, fontweight='bold')
plt.ylabel('Total Views', fontsize=12, fontweight='bold')
plt.title('Linear Regression: First Hour Likes vs Total Views', fontsize=14, fontweight='bold', pad=15)
plt.legend(fontsize=11, frameon=True, fancybox=True, shadow=True)
plt.savefig('Linear Regression_Instagram.png', dpi=150, bbox_inches='tight')
